...
class QueryGenerator {
  ...
  insertQuery(table, valueHash, modelAttributes, options) {
    ...
    if (this._dialect.supports.returnValues && options.returning) {
      if (this._dialect.supports.returnValues.returning) {
        ...
        if (
          modelAttributes &&
          options.hasTrigger &&
          this._dialect.supports.tmpTableTrigger
        ) {
          let tmpColumns = "";
          let outputColumns = "";
          ...
          for (const modelKey in modelAttributes) {
            ...
            if (!(attribute.type instanceof DataTypes.VIRTUAL)) {
              ...
              tmpColumns += `${this.quoteIdentifier(
                attribute.field
    ...
    if (this._dialect.supports.EXCEPTION && options.exception) {
      ...
      if (semver.gte(this.sequelize.options.databaseVersion, "9.2.0")) {
        ...
        valueQuery = `${
          `CREATE OR REPLACE FUNCTION pg_temp.testfunc(OUT response ${quotedTable}, OUT sequelize_caught_exception text) RETURNS RECORD AS ${delimiter}` +
          " BEGIN "
        }${valueQuery} INTO response; EXCEPTION ${
          options.exception
  ...
  bulkInsertQuery(tableName, fieldValueHashes, options, fieldMappedAttributes) {
    ...
    const attributes = allAttributes
      ...
      .join(",");
    const returning =
      this._dialect.supports.returnValues && options.returning
        ? " RETURNING *"
        : "";
    ...
    return `INSERT${ignoreDuplicates} INTO ${this.quoteTable(
      tableName
    )} (${attributes}) VALUES ${tuples.join(
      ","
  ...
  arithmeticQuery(
    ...
    for (const key in attrValueHash) {
      const value = attrValueHash[key];
      values.push(
        `${this.quoteIdentifier(key)}=${this.quoteIdentifier(
          key
        )}${operator} ${this.escape(value)}`
      );
    }
    ...
    return `UPDATE ${this.quoteTable(tableName)} SET ${values.join(
      ","
    )}${outputFragment} ${this.whereQuery(where)} ${returningFragment}`.trim();
  }
  ...
  */
  ...
  removeConstraintQuery(tableName, constraintName) {
    ...
    return `ALTER TABLE ${tableName} DROP CONSTRAINT ${this.quoteIdentifiers(
      constraintName
    )}`;
  }
  ...
  */
      ...
      for (const include of options.include) {
        ...
        if (joinQueries.attributes.subQuery.length > 0) {
          attributes.subQuery = attributes.subQuery.concat(
            joinQueries.attributes.subQuery
          );
        }
      }
    }
  ...
  generateInclude(include, parentTableName, topLevelInfo) {
    ...
    if (topLevelInfo.options.includeIgnoreAttributes !== false) {
      ...
      const includeAttributes = include.attributes.map((attr) => {
        ...
        return `${prefix} AS ${this.quoteIdentifier(
          `${includeAs.externalAs}.${attrAs}`,
          true
        )}`;
      });
  ...
  generateJoin(include, topLevelInfo) {
    ...
    if (
      ...
      if (parentIsTop) {
        ...
        joinOn = `${this.quoteTable(
          parent.as || parent.model.name
        )}.${this.quoteIdentifier(attrLeft)}`;
      } else {
        joinOn = this.quoteIdentifier(
          `${asLeft.replace(/->/g, ".")}.${attrLeft}`
        );
      }
    }
  ...
  generateThroughJoin(include, includeAs, parentTableName, topLevelInfo) {
    ...
    const throughAttributes = through.attributes.map(
      (attr) =>
        `${this.quoteIdentifier(throughAs)}.${this.quoteIdentifier(
          ...
          `${externalThroughAs}.${Array.isArray(attr) ? attr[1] : attr}`
        )}`
    );
    const association = include.association;
    const parentIsTop =
    ...
    const attrTarget =
      association.target.rawAttributes[primaryKeysTarget[0]].field ||
      primaryKeysTarget[0];
    ...
    if (
      ...
      sourceJoinOn = `${this.quoteIdentifier(
  ...
  _generateSubQueryFilter(include, includeAs, topLevelInfo) {
    ...
    topLevelInfo.options.where[`__${includeAs.internalAs}`] =
      this.sequelize.literal(
  ...
  handleSequelizeMethod(smth, tableName, factory, options, prepend) {
    ...
    if (smth instanceof Utils.Where) {
      ...
      if (value && value instanceof Utils.SequelizeMethod) {
        ...
        return value === "NULL"
          ? `${key} IS NULL`
          : [key, value].join(` ${smth.comparator} `);
      }
  ...
  whereItemQuery(key, value, options = {}) {
    ...
    if (typeof key === "string" && key.includes(".") && options.model) {
      ...
      if (
        ...
        return this.whereItemQuery(
          field.field || keyParts[0],
          tmp,
    ...
    Utils.getOperators(value).forEach((op) => {
      ...
      items.push(
        this.whereItemQuery(
          key,
          where,
          Object.assign({}, options, { json: false })
        )
      );
    });
